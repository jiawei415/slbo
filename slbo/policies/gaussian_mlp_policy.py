...
class GaussianMLPPolicy(nn.Module, BasePolicy):
    ...
    def __init__(
        ...
        with self.scope:
            ...
            self.op_actions_ = tf.placeholder(
                tf.float32, shape=[None, dim_action], name="actions"
            )
            ...
            self.op_log_std = nn.Parameter(
                tf.constant(
                    np.log(self.init_std), shape=[self.dim_action], dtype=tf.float32
                ),
                name="log_std",
            )
    ...
    def fast(self, states, use_log_prob=False):
        ...
        if use_log_prob:
            log_prob = (
                -(noise**2) / 2 - np.log(2 * np.pi) / 2 - self.op_log_std.numpy()
            )
