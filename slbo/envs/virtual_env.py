...
class VirtualEnv(BaseBatchedEnv):
    ...
    def __init__(
        self, model: DynamicsModel, env: BaseModelBasedEnv, n_envs: int, opt_model=False
    ):
        super().__init__()
        self.n_envs = n_envs
        ...
        if opt_model:
            self.action_space = Box(
                low=np.r_[env.action_space.low, np.zeros(dim_state) - 1.0],
                high=np.r_[env.action_space.high, np.zeros(dim_state) + 1.0],
                dtype=np.float32,
            )
        else:
    ...
    def _scale_action(self, actions):
        ...
        return lo + (actions + 1.0) * 0.5 * (hi - lo)
    ...
    def step(self, actions):
        ...
        if len(actions.shape) == 1:
            one_hot_action = np.zeros((actions.shape[0], self.action_space.n))
            one_hot_action[np.arange(actions.shape[0]), actions] = 1
            next_states = self._model.eval(
                "next_states", states=self._states, actions=one_hot_action
            )
        else:
            next_states = self._model.eval(
                "next_states", states=self._states, actions=actions
            )
        rewards, dones = self._env.mb_step(
            self._states, self._scale_action(actions), next_states
        )
