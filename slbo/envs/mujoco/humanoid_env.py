...
class HumanoidEnv(simple_humanoid_env.SimpleHumanoidEnv, BaseModelBasedEnv):
    def __init__(
        self,
        version="v1",
        vel_deviation_cost_coeff=0.01,
        alive_bonus=0.2,
        ctrl_cost_coeff=0.001,
        impact_cost_coeff=0.00001,
        *args,
        **kwargs
    ):
        super().__init__(
    ...
    def step(self, action):
        ...
        impact_cost = 0.0
        vel_deviation_cost = (
            0.5 * self.vel_deviation_cost_coeff * np.sum(np.square(comvel[1:]))
        )
        reward = (
            lin_vel_reward + alive_bonus - ctrl_cost - impact_cost - vel_deviation_cost
        )
    ...
    def mb_step(self, states, actions, next_states):
        ...
        ctrl_cost = 5.0e-4 * np.square(actions / scaling).sum(axis=1)
        impact_cost = 0.0
        vel_deviation_cost = 5.0e-3 * np.square(next_states[:, 37:39]).sum(axis=1)
        reward = (
            lin_vel_reward + alive_bonus - ctrl_cost - impact_cost - vel_deviation_cost
        )
